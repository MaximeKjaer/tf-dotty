...
object Example extends App {
  ...
  val yTensor = tf.constant[Seq[Float], Float, 5000 #: SNil.type](
    yData,
    tf.float32,
    5000 #: SNil
  )
  ...
  val x = tf.constant[Seq[Float], Float, 5000 #: SNil.type](
    xData,
    tf.float32,
    5000 #: SNil
  )
  val y = (tf.broadcast_to(W, 5000 #: SNil) * x) + tf.broadcast_to(
    b,
    5000 #: SNil
  )
